...
fn load_struct_handles(
    ...
    while cursor.position() < table.count as u64 {
        ...
        let type_parameters = load_type_parameters(&mut cursor)?;
...
fn load_type_parameters(
    ...
    for _ in 0..len {
        type_params.push(load_type_parameter(cursor)?);
    }
    Ok(type_params)
}

fn load_type_parameter(cursor: &mut VersionedCursor) -> BinaryLoaderResult<StructTypeParameter> {
